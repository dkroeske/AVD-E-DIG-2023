...
void handleArtDiscovery(uint8_t *data, struct ArtPollReply *reply) {
    ...
    reply->SwOut[0] = ARTNET_UNIVERSE;            // Xlights presents universe 
}
        ...
        case 0x5000: {
            ...
            if( (dmx.SubUni >= universe) && (dmx.SubUni < (universe+ARTNET_MAX_UNIVERSES)) ) {
                ...
                if( p_sequence != dmx.Sequence ) {
                    ...
                    unsigned int nr_leds = ARTNET_NUMBER_LEDS;
 ...
 * Init UDP interface
 */
uint8_t initArtNet( void(*cb)(void *) )
{
    // Save callback 
    onDataAvailable = cb;
